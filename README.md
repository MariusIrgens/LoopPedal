...
A full sequence is made from concatenating 8 patterns. At the first pattern of a new sequence, a cymbal crash is added to the first step. At the last pattern, a tom fill is added, spanning the last two beats. After a tom fill, a new sequence is generated using the same template, making changes to the beat but staying within the rules of the template. This results in an ever changing beat. Pattern 2 and 6 are a bit busyer, while pattern 4 and 8 are the busyest - drummers will often add hits to the sequence in this way to make it easier to orient in it, and add exitement in the form of rythmic "dissonance" and "resolution".
